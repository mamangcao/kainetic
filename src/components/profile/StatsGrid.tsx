...
const StatsGrid: React.FC<StatsGridProps> = ({ sport, fetcher }) => {
  ...
  // Default goals logic
  const defaultWeeklyGoal = sport === 'cycling' ? 150 : (sport === 'walking' ? 20 : 40);
  const defaultMonthlyGoal = sport === 'cycling' ? 600 : (sport === 'walking' ? 80 : 160);

  useEffect(() => {
    setStats(null);
    fetcher(sport).then(setStats);
    setGoals({ weekly: defaultWeeklyGoal, monthly: defaultMonthlyGoal });
  }, [sport, fetcher, defaultWeeklyGoal, defaultMonthlyGoal]);
  ...
  const StatCell = ({ 
    ...
    <div className={`p-5 flex flex-col justify-between h-full ${className} group hover:bg-white/[0.02] transition-colors relative`}>
      ...
      {growth !== undefined && addedValue !== undefined && (
        <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
            <span className="text-[9px] text-slate-600 font-medium">Since last year</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <Card className="p-0 overflow-hidden border border-white/5 bg-[#1c2229] flex flex-col h-full shadow-lg shadow-black/20">
          <div className="px-5 py-3 border-b border-white/5 bg-gradient-to-r from-white/[0.03] to-transparent flex items-center justify-between">
            ...
            <div className="px-2 py-0.5 bg-blue-500/10 border border-blue-500/20 rounded text-[10px] font-bold text-blue-400">Since 2018</div>
          </div>
