      ...
      >
        ...
        <div className="relative z-10 group">
          <div className="w-24 h-24 rounded-full p-[3px] bg-gradient-to-br from-blue-500 to-lime-400 shadow-lg shadow-blue-500/20">
             <img src={profile.profile_medium} alt={profile.username} className="w-full h-full rounded-full object-cover border-4 border-[#1c2229]" />
          </div>
        </div>
        ...
        <div className="flex-1 relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-1">
            ...
            {stats && activeTab === 'running' && (
              <button 
                ...
                className="flex items-center justify-center gap-2 px-4 py-2 bg-lime-500 hover:bg-lime-400 text-[#101418] rounded-xl font-bold text-sm shadow-lg shadow-lime-500/20 transition-all active:scale-95 group/btn cursor-pointer"
              >
                <FileText size={16} />
                Generate Run Proof
          ...
          <div className="flex gap-2 justify-center md:justify-start flex-wrap">
               {['Running', 'Cycling', 'Hiking'].map((tag) => (
                  <div key={tag} className="px-3 py-1 bg-[#101418] text-slate-300 text-xs font-semibold rounded-lg border border-white/10 hover:border-blue-500/50 hover:text-blue-400 transition-colors cursor-default">
                    {tag}
                  </div>
               ))}
          </div>
