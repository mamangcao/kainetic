...
export const getStats = async (sport: SportType): Promise<StatsData> => {
  ...
  const calculateSteps = (km: number) =>
    sport === "walking" ? Math.floor(km * 1350) : undefined;
  ...
  const getHighlights = (period: "week" | "month"): ActivityHighlight[] => {
    ...
    if (sport === "walking") {
      ...
      const paceS = Math.floor(Math.random() * 60)
        .toString()
        .padStart(2, "0");
    ...
    const longestVal = `${(distBase * (0.8 + Math.random() * 0.4)).toFixed(
      1
    )} km`;
    const pm = 5 + Math.floor(Math.random() * 2);
    const ps = Math.floor(Math.random() * 60)
      .toString()
      .padStart(2, "0");
    ...
    return [
      ...
      { label: "Fastest 15km", value: generateTime(t15, v), icon: "timer" },
      { label: "Fastest 21km", value: generateTime(t21, v), icon: "timer" },
      { label: "Fastest 30km", value: generateTime(t30, v), icon: "timer" },
      { label: "Fastest 42km", value: generateTime(t42, v), icon: "timer" },
    ];
  };
  ...
  return {
    allTime: {
      ...
      growth: {
        ...
        steps:
          stepsAll && stepsYtd
            ? calculateGrowth(stepsAll, stepsYtd)
            : undefined,
      },
    },
    ytd: {
    ...
    recent: {
      thisWeek: {
        ...
        time: `${Math.floor(distThisWeek / 9)}h ${Math.floor(
          (distThisWeek % 9) * 6
        )}m`,
      ...
      thisMonth: {
        ...
        time: `${Math.floor(distThisMonth / 9)}h ${Math.floor(
          (distThisMonth % 9) * 6
        )}m`,
...
export const getStory = async (
  ...
  if (sport === "cycling") {
    return {
      summary: `Looks like you're putting in some serious mileage! The AI detected a strong improvement in your climbing speed this ${periodText}. Keep up this momentum.`,
      insights: [
        {
          label: "Best 5 Min Power",
          value: "280W",
          detail: "Maintained during Saturday's ride",
          icon: "zap",
          color: "text-yellow-400",
          bgColor: "bg-yellow-500/10",
        },
        {
          label: "Toughest Climb",
          value: "320m",
          detail: "Avg 7% grade • Peak HR 175",
          icon: "mountain",
          color: "text-red-400",
        ...
        },
        {
          label: "Most Improved",
          value: "Cadence",
          detail: `Avg 85rpm (+5 vs last ${periodText})`,
          icon: "trending-up",
        ...
        },
        {
          label: "Weather Hero",
          value: "Windy",
          detail: "Battled 30km/h gusts on Tuesday",
          icon: "wind",
          color: "text-cyan-400",
  ...
  if (sport === "walking") {
    return {
      summary: `Slow and steady wins the race. You've been extremely consistent with your daily steps. That long walk on Sunday really boosted your ${periodText}ly total.`,
      insights: [
        {
          label: "Top Step Count",
          value: "15,200",
          detail: "Sunday • Nature Trail",
          icon: "footprints",
        ...
        },
        {
          label: "Active Time",
          value: "5h 30m",
          detail: `Total duration this ${periodText}`,
          icon: "heart",
          color: "text-pink-400",
        ...
        },
        {
          label: "Discovery",
          value: "New Route",
          detail: "Explored 'River Path' segment",
          icon: "map",
          color: "text-blue-400",
  ...
  return {
    ...
    insights: [
      ...
      },
      {
        label: "Efficiency",
        value: "+3%",
        detail: "Lower HR at aerobic pace",
        icon: "heart",
        color: "text-rose-400",
      ...
      },
      {
        label: "Weather Hero",
        value: "Rain",
        detail: "Completed run in heavy rain",
        icon: "cloud-rain",
...
export const getActivitiesHeatmap = async (
  ...
  for (let i = 364; i >= 0; i--) {
    ...
    const chance = isWeekend ? 0.6 : 0.3;

    if (Math.random() < chance) {
      // It's a workout day!
      const intensity = Math.ceil(Math.random() * 4);
      const base = sport === "cycling" ? 20000 : 5000;
      const distance = base * intensity * (0.8 + Math.random() * 0.4);
...
export const getChartData = async (
  ...
  const getActivities = (dist: number) => {
  ...
  const getElevation = (dist: number) =>
    Math.floor(dist * (sport === "cycling" ? 10 : 6) + Math.random() * 30);
  ...
  if (period === "week") {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const dist =
        Math.floor(Math.random() * (sport === "cycling" ? 150 : 50)) + 10;

      weeks.push({
        name: startStr,
        value: dist,
        range: `${startStr} - ${endStr}`,
        activities: getActivities(dist),
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const dist =
        Math.floor(Math.random() * (sport === "cycling" ? 600 : 150)) + 50;
...
export const getBestEfforts = async (
  sport: SportType
): Promise<BestEffort[]> => {
  void sport;
  await delay(600);
